...
--                    max_hear_distance = 15,
 ...
 --                   pitch = 1.0})
	for i=1,9 do
    if not rweapons_enable_sparks == "false" then
...
rangedweapons_switch_fire = function(itemstack, player)
    GunCaps = itemstack:get_definition().RW_gun_capabilities
    itemstack:set_name(GunCaps.gun_switch)
end
...
gun_recoil = GunCaps.gun_recoil or 1
...
bullet_recoil = AmmoCaps.ammo_recoil or 1
...
rangedweapons_launch_projectile(player,combined_projNum,gun_damage,bullet_ent,bullet_visual,bullet_texture,gun_sound,combined_velocity,gun_accuracy,skill_value,OnCollision,combined_crit,combined_critEffc,combined_mobPen,combined_nodePen,gun_shell,bullet_shell_ent,bullet_shell_texture,bullet_shell_visual,combined_dps,combined_gravity,gun_door_breaking,bullet_glass_breaking,bullet_particles,bullet_sparks,bullet_bomb_ignite,bullet_size,gun_smokeSize,0,bullet_glow)

if rweapons_gun_recoil == "true" then

local function recoil(player)
    local currentYaw = player:get_look_horizontal()
    local currentPitch = player:get_look_vertical()
    player:set_look_horizontal(currentYaw + gun_recoil / math.random() * 0.000250 - 0.000125 )
    player:set_look_vertical(currentPitch - gun_recoil / 200)
end
...
rangedweapons_launch_projectile(player,power_projectiles,power_damage,power_ent,power_visual,power_texture,power_sound,power_velocity,power_accuracy,skill_value,OnCollision,power_crit,power_critEffc,power_mobPen+20,power_nodePen,0,"","","",power_dps,power_gravity,power_door_breaking,power_glass_breaking,power_particles,power_sparks,power_bomb_ignite,power_size,0,0,power_glow)
...
----------------------------------
    		...
    		if pos and dir and yaw then
    		minetest.sound_play(shoot_sound, {
                    pos = player:get_pos(),
                    max_hear_distance = 15,
                    gain = 1.0,
                    fade = 0.0,
    ...
    if not rweapons_enable_gun_smoke == "false" then
    if smokeSize > 0 then
    	minetest.add_particle({
    		pos = pos,
...
if rweapons_handguns == "true" then
    ...
    dofile(modpath.."/dragoon.lua")
    dofile(modpath.."/rugeriv.lua")
    dofile(modpath.."/glock44.lua")
    dofile(modpath.."/glock40.lua")
    dofile(modpath.."/fiveseven.lua")
end
